...
module basin_grid
    ...
    type(block2D), dimension(:), pointer :: lu,        &  !mask of t-grid
    ...
    type(block2D), dimension(:), pointer :: hhh,     &  !ocean depth on luh (h-points)
    ...
    type(block2D), dimension(:), pointer :: xt,yt,  &  !horizontal t-grid            x- and y-coordinates (in degrees)
                                            xu,yv      !horizontal u-grid and v-grid x- and y-coordinates (in degrees)

    type(block2D), dimension(:), pointer ::  geo_lon_t,   &    !geographical longitudes of T-points
