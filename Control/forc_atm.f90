...
!   initialization of atmospheric sea-land mask      
    ...
    if (rank == 0) write(*,*) 'building matrix for interpolation from atm to ocean'
    
    !  calculating interpolation matrix elements   
    do k = 1, bcount
        call set_block(k)
        call set_block_lu(k)
        call set_block_geo(k)
        call weight_matrix_intrp_next(xa,       & !array(1-D) of input x-grid values (input) 
                            ...
                            i_input_a2o(k)%vals,        & !x-grid numbers of input grid for output grid (output)
                            j_input_a2o(k)%vals,        & !y-grid numbers of input grid for output grid (output)
                          wght_mtrx_a2o(k)%vals,        & !nonzero matrix elements of interpolation (output)
   ...
   if(ind_change_stress>0) then
        ...
        call interpolrot_vec(nxa,         &  !number of x-grid points(input)
                       ...
                       a_stress_y,        &  !input meridional data array
                             taux(k)%vals,        &  !output zonal data array
                             tauy(k)%vals,        &  !output meridional data array
                      int(i_input_a2o(k)%vals),        &  !x-grid numbers of input grid for output grid
                      int(j_input_a2o(k)%vals),        &  !y-grid numbers of input grid for output grid
                      wght_mtrx_a2o(k)%vals,      &  !nonzero matrix elements of interpolation
      ...
      call interpolrot_scal(nxa,         &  !number of x-grid points(input)
                         ...
                         bnd_y1,bnd_y2,  &  !number of y-grid points(output)
                               a_hflux,  &  !input data array
                                hf_tot(k)%vals,  &  !output data array
                           int(i_input_a2o(k)%vals),  &  !x-grid numbers of input grid for output grid
                           int(j_input_a2o(k)%vals),  &  !y-grid numbers of input grid for output grid
                         wght_mtrx_a2o(k)%vals,  &  !nonzero matrix elements of interpolation
                                    lu,  &  !sea-land mask for output data
      ...
      call interpolrot_scal(nxa,         &  !number of x-grid points(input)
                         ...
                         bnd_y1,bnd_y2,  &  !number of y-grid points(output)
                               a_swrad,  &  !input data array
                                sw_bal(k)%vals,  &  !output data array
                           int(i_input_a2o(k)%vals),  &  !x-grid numbers of input grid for output grid
                           int(j_input_a2o(k)%vals),  &  !y-grid numbers of input grid for output grid
                         wght_mtrx_a2o(k)%vals,  &  !nonzero matrix elements of interpolation
                                    lu,  &  !sea-land mask for output data
      ...
      call interpolrot_scal(nxa,         &  !number of x-grid points(input)
                         ...
                         bnd_y1,bnd_y2,  &  !number of y-grid points(output)
                               a_wflux,  &  !input data array
                                wf_tot(k)%vals,  &  !output data array
                           int(i_input_a2o(k)%vals),  &  !x-grid numbers of input grid for output grid
                           int(j_input_a2o(k)%vals),  &  !y-grid numbers of input grid for output grid
                         wght_mtrx_a2o(k)%vals,  &  !nonzero matrix elements of interpolation
                                    lu,  &  !sea-land mask for output data
...
!----------------------------------------------------------------------------------------
 if(iabs(ksw_ssbc)>=3) then
  ...
  if(ind_change_wind>0) then
    ...
    call interpolrot_vec(nxa,         &  !number of x-grid points(input)
                      ...
                      bnd_y1,bnd_y2,  &  !number of y-grid points(output)
                       ...
                       a_vwnd,        &  !input meridional data array
                         uwnd(k)%vals,        &  !output zonal data array
                         vwnd(k)%vals,        &  !output meridional data array
                  int(i_input_a2o(k)%vals),        &  !x-grid numbers of input grid for output grid
                  int(j_input_a2o(k)%vals),        &  !y-grid numbers of input grid for output grid
                  wght_mtrx_a2o(k)%vals,      &  !nonzero matrix elements of interpolation
                    rotvec_coeff,     &  !angles between parallels
    ...
    do n=ny_start-1,ny_end+1
      do m=nx_start-1, nx_end+1
        if (lu(m,n)>0.5) then
          wind(k)%vals(m,n) = sqrt(uwnd(k)%vals(m,n)*uwnd(k)%vals(m,n) + vwnd(k)%vals(m,n)*vwnd(k)%vals(m,n))
        endif             
      enddo
    enddo
  endif    
  ...
  if (prec_split==0 .and. (ind_change_tatm>0 .or. ind_change_prec>0)) then
    if (rank == 0 .and. k == 1) write(*,*) 'Getting snow data from total precipitation'
      !if rain and snow are mixed
      do n=ny_start-1, ny_end+1
        do m=nx_start-1, nx_end+1
          if (lu(m,n)>0.5) then
            rain(k)%vals(m,n) = rain(k)%vals(m,n) + snow(k)%vals(m,n)
            snow(k)%vals(m,n) = 0.0
            if (tatm(k)%vals(m,n) < 0.0) then
              snow(k)%vals(m,n) = rain(k)%vals(m,n)
              rain(k)%vals(m,n) = 0.0
            endif
          endif
        enddo
